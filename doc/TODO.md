...
- [x] ConfigManager 能够正确解析 config.toml 文件
- [x] validate() 方法能够校验所有配置项的合法性（数值范围、类型检查）
- [x] watch() 方法能够监听文件变化并触发回调
- [x] get() 方法返回只读副本，无法被外部修改
- [x] update() 方法能够更新配置并触发 configUpdated 事件
- [x] 单元测试覆盖 ConfigManager 的核心方法
...
- [x] Logger 支持所有 5 个日志级别
- [x] 控制台输出使用正确的颜色（DEBUG 灰色、INFO 白色、WARN 黄色、ERROR 红色、TRADE 蓝色）
- [x] 日志文件正确创建在 logs/ 目录
- [x] 日志文件命名符合规范（精确到秒）
- [x] 日志格式符合规范，包含时间戳、级别、模块名、消息和 JSON 元数据
- [x] setLevel() 能够动态调整日志级别
- [x] close() 方法正确关闭文件句柄
...
实现以**成交事件驱动**的做市主循环，完整覆盖初始化、启动模式、做市模式与状态流转，流程与函数设计严格符合开发规范。

### 6.1 主循环流程对子阶段

#### 关键流程

- 启动后完成初始化（加载配置、初始化日志、连接 SDK、启动预言机监听、检查持仓）
- 无仓位进入启动模式：记录预言机价格 P1，挂买单 B1（Oracle - spread），开启 30 秒计时
- 30 秒后未完全成交：若当前预言机价 > P1 则取消并重新挂单；否则保持等待
- B1 部分成交：取消未成交部分并按相同逻辑重新挂单
- B1 完全成交：进入做市模式
- 做市模式：挂买单 B1（post-only）与卖单 S1（post-only + reduceOnly）
- 任一订单完全成交：立即取消另一侧订单，本轮结束并判断是否有持仓

#### 验收标准

- [x] 主循环完全由成交事件驱动
- [x] 启动模式 30 秒超时重挂逻辑与规范一致
- [x] 订单部分成交会取消剩余并重挂
- [x] 做市模式任一订单成交后取消对手单并结束本轮
...
| 6.2.9  | 实现持仓获取       | 实现 getCurrentPosition() 方法，获取当前持仓               |
| 6.2.10 | 实现状态同步       | 实现 syncState() 方法，同步链上状态                        |
| 6.2.11 | 实现状态机流转     | 实现完整的状态流转逻辑（IDLE -> STARTUP -> MARKET_MAKING） |

#### 验收标准

- [x] start() 方法正确完成所有初始化步骤
- [x] stop() 方法正确清理所有资源（取消订单、关闭连接等）
- [x] runStartupMode() 在无仓位时自动挂买单
- [x] runMarketMakingMode() 在有仓位时同时挂买单和只减仓卖单
- [x] 买单使用 post-only，不吃单
- [x] 卖单使用 reduceOnly，只减仓不平仓
- [x] 卖单数量 = min(可平数量, baseOrderSize)
- [x] 订单成交后自动取消对手方订单
- [x] 30秒超时逻辑正常工作
- [x] 预言机偏移订单能够跟随价格浮动
- [x] 能够处理订单被部分成交的情况
...
### 6.3 EventHandler 子阶段

#### 子任务列表

| 序号  | 子任务             | 详细描述                                       |
| ----- | ------------------ | ---------------------------------------------- |
| 6.3.1 | 实现订单成交处理   | 实现 onOrderFill() 方法，处理订单成交事件      |
| 6.3.2 | 实现订单取消处理   | 实现 onOrderCancel() 方法，处理订单取消事件    |
| 6.3.3 | 实现仓位变化处理   | 实现 onPositionChange() 方法，处理仓位变化事件 |
| 6.3.4 | 实现预言机更新处理 | 实现 onOracleUpdate() 方法，处理预言机价格更新 |

#### 验收标准

- [x] onOrderFill() 正确处理订单成交事件
- [x] onOrderCancel() 正确处理订单取消事件
- [x] onPositionChange() 正确处理仓位变化事件
- [x] onOracleUpdate() 正确处理预言机价格更新
